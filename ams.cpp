...
void Revenue::receipt(DataIter const &n_obj, int const &quantity)
{
    ...
    cout << setw(10) << n_obj->getCompany() << setw(15) << n_obj->getModelName()
         << setw(15) << n_obj->getCost() << setw(15) << quantity << setw(15) << m_gst * 100 << setw(15)
         << m_roadtax * 100 << setw(15) << n_obj->getProfitMargin() << setw(15) << finalCost << '\n';
}
namespace rev
{
    void help()
    ...
    void disp()
    {
        db.dispRev(db.getSales(), db.getProfit());
    }
}
